  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50">
      ...
      <div className="mx-auto max-w-4xl mt-4">
        <div className="relative flex items-center justify-between bg-slate-950/80 backdrop-blur-md border border-slate-800 rounded-full px-6 py-3 shadow-[0_0_20px_rgba(0,0,0,0.5)] ring-1 ring-white/10">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button
              variant="outline"
              size="icon"
              className="rounded-full border-purple-700/50 bg-purple-900/30 hover:bg-purple-800/50 text-purple-300 hover:text-purple-200 relative"
              onClick={onTechClick}
              title="Tech Tree (T)"
            >
              <FlaskConical className="h-4 w-4" />
              <span className="absolute -bottom-1 -right-1 text-[9px] bg-purple-800 text-purple-200 px-1 rounded font-mono">T</span>
            </Button>
            <Button
              variant="outline"
              size="icon"
              className="rounded-full border-slate-700/50 bg-slate-800/30 hover:bg-slate-700/50 text-slate-400 hover:text-white relative"
              onClick={onHelpClick}
              title="How to Play (H)"
            >
        ...
        {cameraPosition && (
          <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-24 h-1 bg-slate-800 rounded-full overflow-hidden" title={`Position: ${cameraPosition.x}/${cameraPosition.width}`}>
            <div
              className="h-full bg-emerald-500/60 transition-all duration-300"
              style={{ width: '20%', marginLeft: `${(cameraPosition.x / cameraPosition.width) * 80}%` }}
            />
          </div>
        )}
      </div>
